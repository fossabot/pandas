...
func (o *SetRootRuleChainURL) Build() (*url.URL, error) {
	...
	var _path = "/rulechains/{ruleChainId}/root"

	ruleChainID := o.RuleChainID
	if ruleChainID != "" {
		_path = strings.Replace(_path, "{ruleChainId}", ruleChainID, -1)
	} else {
		return nil, errors.New("ruleChainId is required on SetRootRuleChainURL")
	}
...
func (o *SetRootRuleChainURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on SetRootRuleChainURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on SetRootRuleChainURL")
	}
