...
func (o *DeleteModelURL) Build() (*url.URL, error) {
	...
	var _path = "/models/{modelId}"

	modelID := o.ModelID
	if modelID != "" {
		_path = strings.Replace(_path, "{modelId}", modelID, -1)
	} else {
		return nil, errors.New("modelId is required on DeleteModelURL")
	}
...
func (o *DeleteModelURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteModelURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteModelURL")
	}
