...
func (o *GetProjectSummaryURL) Build() (*url.URL, error) {
	...
	var _path = "/projects/{projectId}/summary"

	projectID := o.ProjectID
	if projectID != "" {
		_path = strings.Replace(_path, "{projectId}", projectID, -1)
	} else {
		return nil, errors.New("projectId is required on GetProjectSummaryURL")
	}
...
func (o *GetProjectSummaryURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetProjectSummaryURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetProjectSummaryURL")
	}
