...
func (o *GetDeviceLogURL) Build() (*url.URL, error) {
	...
	var _path = "/logs/devices/{deviceId}"

	deviceID := o.DeviceID
	if deviceID != "" {
		_path = strings.Replace(_path, "{deviceId}", deviceID, -1)
	} else {
		return nil, errors.New("deviceId is required on GetDeviceLogURL")
	}
...
func (o *GetDeviceLogURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetDeviceLogURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetDeviceLogURL")
	}
