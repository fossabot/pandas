...
func (o *GetProjectLogURL) Build() (*url.URL, error) {
	...
	var _path = "/logs/projects/{projectId}"

	projectID := o.ProjectID
	if projectID != "" {
		_path = strings.Replace(_path, "{projectId}", projectID, -1)
	} else {
		return nil, errors.New("projectId is required on GetProjectLogURL")
	}
...
func (o *GetProjectLogURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetProjectLogURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetProjectLogURL")
	}
